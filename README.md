...
git init リポジトリ名　と入力すると今いるディレクトリに新たにローカルリポジトリを作成する。
・git add – ファイル追加コマンド<br>
git addファイル名　を入力するとリポジトリに入力したファイルが追加される。
・git commit – 追加及び変更したファイルをgitに登録するためのコマンド 
git commit -m”残したいコメント” と入力するとファイルがgitに登録され、その際に変更点などをコメントに残すことでわかりやすく管理することができる。
＊Git commitの後に発生したエラー
Untracked files:
...
・git push – ローカルリポジトリをリモートリポジトリにアップロードするためのコマンド
